 ...
 */
 ...
 //   • "*.{htm,html}" will find files with an extention of either "htm" or "html"
function globFileList(cwd, pattern) {
  const isDeep = pattern.startsWith('**/');
  if (isDeep) {
    pattern = pattern.slice(3);
  }
  let regexPattern = pattern.split(/\*+/).map(p => p.replace(/([.+^$()|[\]\\])/g, '\\$1')).join('.*')
    .replace('?', '.')
    .replace(/\{([^\}]+)\}/g, (match, options) => `(${options.split(',').join('|')})`);
