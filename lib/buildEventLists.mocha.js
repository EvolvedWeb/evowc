...
describe('getCP1Code tests', () => {
  it('should handle no CP1 codes', () => {
    ...
    expect(resp).to.eql({ evt: 'click', evtOptions: {unique: true}});
  });

  it('should handle the C code', () => {
    const resp = getCP1Code('click:c');
    expect(resp).to.eql({
      evt: 'click', evtOptions: { unique: false, capture: true } });
  });

  it('should handle the P code', () => {
    const resp = getCP1Code('click:p');
    expect(resp).to.eql({
      evt: 'click', evtOptions: { unique: true, passive: true }
    });
  });
  ...
  it('should handle the 1 code', () => {
    ...
    expect(resp).to.eql({
      evt: 'click', evtOptions: { unique: true, once: true }
    });
  });
  ...
  it('should handle the CP1 codes', () => {
    ...
    expect(resp).to.eql({
      evt: 'click', evtOptions: { unique: true, capture: true, passive: true, once: true }
    });
...
describe('buildEventLists tests', () => {
  ...
  describe('inside a for loop', () => {
    ...
    it('should process CP1 on an event', () => {
      ...
      const expectedFor = [
        "ael( el0, 'submit', (evt)=>this.#changePw(evt, {...evt.target.dataset}, idx, item))",
      ]
      const resp = buildEventLists(list)
      expect(resp).to.eql(expectedResp);
