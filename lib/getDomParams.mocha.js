...
describe('getDomParams tests', () => {
  ...
  it('should return properly for just css', () => {
    expect(getDomParams(false, true, false)).to.equal('{appendStylesFn,componentName}');
  });
  ...
  it('should return properly for all three', () => {
    expect(getDomParams(true, true, 'none')).to.equal('{template,appendStylesFn,shadowMode:\'none\',componentName}');
  });
});
