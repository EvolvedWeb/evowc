...
const propLookup = {
  ...
  let properties = `\n\n  //**********************************************************************************\n  // Properties\n`+ props.map(([prop, details]) => {
    ...
    Object.entries(conditionals).forEach(([element, conditional]) => {
      ...
      if (conditional.variable === prop) {
        ...
        if (conditional.value) {
          ...
          conditionalHandlers.push(`handleCondition(this.#els.${element}, value === ${val}, this.#els.${element}_c );`);
        }
        else {
          conditionalHandlers.push(`handleCondition(this.#els.${element}, ${conditional.isTruthy ? 'value' : '!value'}, this.#els.${element}_c );`);
        }
    ...
    return `  get ${transformAttributeName(prop)}() {
    return this.#${transformAttributeName(prop).replace(/#/, '_')};
  }
  set ${transformAttributeName(prop)}(value) {
    value = ${beforeType};
    if (value !== this.#${transformAttributeName(prop).replace(/#/, '_') }) {
      this.#${transformAttributeName(prop).replace(/#/, '_')} = value;${updaters}${condHandlers}
      this.#callUpdate('${prop}');
    }
  }
`;
  }).join('\n');
...
export class ${component.className} extends EvoElement {
  ...
  #callUpdate(field) {
    this.isConnected && this.update && this.update(field);
