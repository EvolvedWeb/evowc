...
const ATTR_KEY = ':@';
const FILE_READ_OPTIONS = { encoding: 'utf-8' };
...
//const NEW_SWITCH_VAR_RE = /^\s*(?<negative>!)?(?<variable>[^:]+)\s*(?::\s*(?:(?<min>\d+)\s*,\s*(?<max>\d+)|(?<value>[^\n]+)))?$/;
...
export class Component {
   ...
   */
  constructor(attrs, children = [], options = {}) {
    //console.log(attrs);
  ...
  #parseComponentParts(children, availableVars) {
    ...
    children.forEach(child => {
      if (child.style) {
        const { src } = child[ATTR_KEY] || {};
        let data = (child.style[0]?.$ || '').trim();
        if (src) {
          if (data) {
            throw new Error(`You can not have an external style file and content in the same "<style>" tag in the file "${this.#options.srcName}".`)
          }

          const fname = path.join(this.#options.srcPath, src)
          if (!fs.existsSync(fname)) {
            throw new Error(`Unable to find the external style file "${src}" in the file "${this.#options.srcName}"`)
          }

          // @ts-ignore
          data = fs.readFileSync(fname, FILE_READ_OPTIONS);
        }
        ...
        //const isServer = child[ATTR_KEY]?.server;
        const { root: isRoot, src } = child[ATTR_KEY] || {};
        let data = (child.script[0]?.$ || '').trim();
        if (src) {
          if (data) {
            throw new Error(`You can not have an external script file and content in the same "<script>" tag. in the file "${this.#options.srcName}"`)
          }

          const fname = path.join(this.#options.srcPath, src)
          if (!fs.existsSync(fname)) {
            throw new Error(`Unable to find the external JavaScript file "${src}" in the file "${this.#options.srcName}"`)
          }
        ...
        if (data) {
          scripts[isRoot ? 'rootScript' : 'classScript'].push(data);
        }
      }
        ...
        const newForInfo = {};
        const forLoopStack = [];
        const { tag, attributes } = this.#getTagAttributesAndChildren(child);

        // Process binding attributes and events for the component
        const parsedTemplateAttrs = this.#parseTemplateAttrs(tag, attributes, newForInfo, availableVars, forLoopStack );
        const {
          attrs,
          bindings,
          conditional,
        ...
        if (conditional) {
          throw new Error(`You can not use '$if' or '$switch' on the template element ${elementStr} in the file "${this.#options.srcName}"`);
        }

        if (forLoop) {
          throw new Error(`You can not use '$for' on the template element ${elementStr} in the file "${this.#options.srcName}"`);
        }

        if (bindings) {
          bindings.forEach(binding => {
            if (binding.prop) {
              throw new Error(`You can not bind to properties on the template element ${elementStr} in the file "${this.#options.srcName}"`);
            }
          })
        }
        this.#addBindingsToProperty('this', 'this', elementStr, subAvailableVars, bindings);

        if(bindings?.length > 0 || Object.keys(events||{}).length > 0) {
          const templateEl = {
            tag,
            elementName: 'this',
            element: 'this',
    ...
    Object.entries(attributes).forEach(([attr, val]) => {
      ...
      if (attrType === CONDITIONAL_PREFIX) {
        if (attrLower === 'if') {
          ...
          const exec = SWITCH_VAR_RE.exec(val)
          if (!exec) {
            throw new Error(`You must define a comparison value ($switch="<cpa>:<value>") or min and max comparison values ($switch="<cpa>:<min>,<max>") for the switch.`);
          }
          conditional = { ...exec.groups };
          // @ts-ignore
