 ...
 * @prop {object} events
 * @prop {null|ComponentBinding[]} forBindings
...
class Component {
  ...
  #getForInfo(element) {
    ...
    function getForInfo(element) {
      if (typeof element === 'object') {
        const {elementName, bindings, forBindings, forEvents, children } = element
        if (bindings) {
          bindings.forEach(fb => {
            forInfo.bindings.push({ ...fb, elementName });
          });
        }
        if (forBindings) {
          forBindings.forEach(fb => {
            forInfo.bindings.push({ ...fb, elementName });
          });
        }

        if (forEvents) {
          forInfo.events[elementName] = forEvents;
        }

        if (children) {
          children.forEach(el => getForInfo(el));
        }
      }
    }
  ...
  /**
   * Handle forBindings and forEvents by adding them to the appropriate forLoop section
   *
  ...
  #parseComponentParts(children, availableVars) {
    ...
    children.forEach(child => {
      if (child.style) {
        ...
        parts.template = this.#parseTemplate(child.template, availableVars);
        this.#processForLoopDataInPlace(parts.template);
      }
    });

    //console.log(JSON.stringify(parts.template, 0, 2));
  ...
  #parseTemplate(template, availableVars, inForLoop = false) {
    return (template || []).map(el => {
      ...
      const { attrs, bindings, conditional, element, elementName, elementStr,
        events, forEvents, forBindings, forLoop, key, subAvailableVars } = this.#parseTemplateAttrs(tag, el[ATTR_KEY], newForInfo, availableVars, inForLoop);
      ...
      const children = childrenSrc.length > 0 ? this.#parseTemplate(childrenSrc, subAvailableVars, inForLoop || !!forLoop) : [];
   ...
   * @returns 
   */
  #parseTemplateAttrs(tag, attributes, newForInfo, availableVars, inForLoop) {
    const attrs = {};
    let bindings = null;
    let events = null;
    let conditional = null;
    let forBindings = null;
    let forEvents = null;
    let forLoop = null;
    ...
    if (forCommand) {
      ...
      forLoop = newForInfo[item] = { variable, item, key, bindings: null, events: null };
    ...
    Object.entries(attributes).forEach(([attr, val]) => {
      ...
      // :title="titleVar"
        ...
        const fieldType = "''";//this.#properties[]
        const binding = parseBinding(val, element, attribute, type, subAvailableVars, fieldType, inForLoop);
        if (binding.srcObj === 'this') {
          bindings ??= [];
