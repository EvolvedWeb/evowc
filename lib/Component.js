...
class Component {
  ...
  #getForInfo(element) {
    ...
    function getForInfo(element) {
      if (typeof element === 'object') {
        const { element: fullElementName, elementName, elementStr, bindings, events, children } = element
        if (bindings) {
          bindings.forEach(fb => {
            if (fb.inForLoop) {
  ...
  #parseComponentParts(children, availableVars) {
    ...
    children.forEach(child => {
      if (child.style) {
        ...
        parts[root?'rootScript':'classScript'].push(child.script[0]['$']);
      }
  ...
  #parseTemplate(template, availableVars, inForLoop = false, forElementName = '') {
    return (template || []).map(el => {
      ...
      const { attrs, bindings, conditional, element, elementName, elementStr,
        events, forElementName: fen, forLoop, key, subAvailableVars } = this.#parseTemplateAttrs(tag, el[ATTR_KEY], newForInfo, availableVars, inForLoop, forElementName);
      ...
      const children = childrenSrc.length > 0 ? this.#parseTemplate(childrenSrc, subAvailableVars, inForLoop || !!forLoop, forElementName || fen) : [];
   ...
   * @returns 
   */
  #parseTemplateAttrs(tag, attributes, newForInfo, availableVars, inForLoop, forElementName) {
    const attrs = {};
    let bindings = null;
    let events = null;
    ...
    Object.entries(attributes).forEach(([attr, val]) => {
      ...
      // :title="titleVar"
        ...
        const binding = parseBinding(val, element, attribute, type, subAvailableVars, inForLoop, forElementName);
