...
const ATTR_KEY = ':@';
const BINDING_NAME_RE = /^(?:(?<isAttr>attr\.)|(?<isData>data-))?(?<field>\w[\-\d\w]+)$/;
...
class Component {
  ...
  #template = '';
  #extendsName = ''; // Name of the HTML Element class this component extends
  ...
  constructor(attrs, children) {
    ...
    this.#extendsName = attrs.extends || '';
  ...
  #processComponentAttrs(attrs) {
    Object.entries(attrs).forEach(([attr, value]) => {
      ...
      if (attr[0] === PDA_PREFIX) {
        ...
        this.#properties[attrName] = parsePda(attrName, value, alsoSetAttr, isPublic, this.#tag);
      }
      else {
  ...
  #parseTemplateAttrs(tag, attributes, newForInfo) {
    ...
    Object.entries(attributes).forEach(([attr, val]) => {
      ...
      // :title="titleVar"
        ...
        //console.log(resp);
        const { isAttr, isData, field } = resp.groups;
        const type = isAttr ? 'attr' : isData ? 'data' : 'prop';
        bindings.push(parseBinding(val, element, field, type));
        keepElementName = true;
      }
      //****************************************************
      // Event Commands
