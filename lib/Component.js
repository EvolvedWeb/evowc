...
class Component {
  ...
  #parseTemplate(template, availableVars, inForLoop = false, forElementName = '', loopItemHandlerName='', indexName = null) {
    return (template || []).map(el => {
      ...
      const { attrs, bindings, conditional, element, elementName, elementStr,
        events, forElementName: fen, forLoop, key, subAvailableVars, indexName: newIndexName } = this.#parseTemplateAttrs(tag, el[ATTR_KEY], newForInfo, availableVars, inForLoop, forElementName, loopItemHandlerName, indexName);
      ...
      const children = childrenSrc.length > 0 ? this.#parseTemplate(childrenSrc, subAvailableVars, inForLoop || !!forLoop, forElementName || fen, loopItemHandlerName || forLoop?.loopItemHandlerName || '', newIndexName) : [];
   ...
   * @returns 
   */
  #parseTemplateAttrs(tag, attributes, newForInfo, availableVars, inForLoop, forElementName, loopItemHandlerName, indexName) {
    const attrs = {};
    let bindings = null;
    let events = null;
    ...
    if (forCommand) {
      ...
      forLoop = newForInfo[item] = { variable, item, key, indexName, bindings: null, events: null, loopItemHandlerName: `#loopItemHandler_${this.#elementIndex}` };
    ...
    Object.entries(attributes).forEach(([attr, val]) => {
      ...
      */
      ...
      // :title="titleVar"
        ...
        const binding = parseBinding(val, element, attribute, type, subAvailableVars, inForLoop, forElementName, loopItemHandlerName, indexName);
