...
function buildTemplate(templateObj) {
  ...
  if (templateObj) {
    templateObj.forEach(el => {
      const { bindings, element, events, conditionals, addElName } = buildElement(el);
      if (addElName) {
        hasElName = true;
      }
      template += element;
  ...
  return { events: allEvents, template: template.trim(), bindings: allBindings, conditionals: allConditionals, hasElName };
}
...
function buildElement(elData = {}) {
  ...
  if (elData.name) {
    ...
    let elName = elData.attributes.el || `el${elIndex++}`;
    let localBinds = getLocalBindings(elData.bindings, elName);
    ...
    if (Object.keys(elData.events).length > 0) {
      events[elName] = elData.events;
    }

   addElName = elData.attributes.el || Object.keys(localBinds).length || Object.keys(events).length;
    ...
    const el = addElName ? ` el="${elName}"` : '';
    //console.log(el);
    element += `<${elData.name}${attrs?` ${attrs}`:''}${el}>`;

    if(elData.children) {
      elData.children.forEach(el => {
        const { bindings: binds, element: elStr, events: subEvents, conditionals: subConditionals, addElName: subAddElName } = buildElement(el);
        element += elStr;
  ...
  return { bindings, element, events, conditionals, addElName };
}

async function componentToSrc(component, minify, addDebug = false) {
  elIndex = 0; // Reset since each component will have their own indexes.

  const props = Object.entries(component.properties);
  const { template, bindings, events: allEvents, conditionals, hasElName } = buildTemplate(component.component.template);
  const useEls = props.length || hasElName;
