...
`+ props.map(([prop, details]) => {
    ...
    Object.entries(conditionals).forEach(([element, conditional]) => {
      ...
      if (conditional.variable === prop) {
        conditionalHandlers.push(`handleCondition(this.#els.${element}, value, ${conditional.isTruthy ? 'true' : 'false'} );`);
      }
    })
...
async function ribbit(componentFileName, outputScriptPath, options = {}) {
...
/*
if (wrongTag) {
  console.error("Unsupported tags. You can only define <component> at the top level. Invalid elements:")
  console.info(restKeys.join(', '));
}
*/

const files = [
  './components/PortfolioImgElement.html',
  './components/SimpleAddressElement.html',
  './components/SpecialThingElement.html',
  './components/LocalTimeElement.html'
]
async function test() {
  console.time('processing time');
  const outputScriptPath = './public/components/';
  ...
  for(let i = 0; i < files.length; i++) {
    ...
    await ribbit(componentFileName, outputScriptPath, options);
  }
