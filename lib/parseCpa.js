import { TYPE_NORMALIZE, VALID_TYPES } from './enums.js';
import { decodeAttributeValue } from './attributeValue.js';
import { escSingleQuote } from './escSingleQuote.js';
 ...
 * @prop {boolean} isPublic - Indicates if this property is public or not.
 * @prop {string} [max] - Optional max for types `int`, `num`, and `BigInt`
 * @prop {string} [min] - Optional min for types `int`, `num`, and `BigInt`
 ...
 * @prop {string | boolean} value - The default value for this property
 */
...
export function parseCpa(attrName, value, setHostAttr, isPublic, tag) {
  ...
  if (value !== true) {
    ...
    //console.log({ type, min, max, objType });
    /** @type {*} */
    ...
    if (!isVar) {
      ...
      switch (type) {
        ...
        case 'bigint':
          ...
          try {
            val = BigInt(val.replace('n', ''));
          }

          // eslint-disable-next-line no-unused-vars
          catch(ex) {
            throw new TypeError(`\x1B[91mThe value \x1B[97m(${origVal})\x1B[91m for the CPA \x1B[41m\x1B[97m"${attrName}"\x1B[91m\x1B[49m for the component \x1B[97m"${tag}"\x1B[91m was invalid and should have been an integer value.\x1B[39m`);
          }
          break;
    ...
    if (['num', 'int', 'bigint'].includes(type)) {
      cpa.min = min;
      cpa.max = max;
    }
  }
