...
class Generator {
  ...
  async srcFromComponent(component, options) {
    ...
    const [eventList, inputHandlerList] = await this.#buildEventList(template);
...
` : ''; 
  ...
  #getPipedVal(pipes, defaultVal ) {
    if (pipes) {
      return `${[...pipes].reverse().join('(')}(newVal ?? ${defaultVal}${pipes.map(p => ')').join('')}`;
    }
...
${classScript}`;
    ...
    usesObjects && imports.push('sameObjs');
    usesDates && imports.push('sameDates');
  ...
  #processPropertyScript(properties) {
    const temp = Object.entries(properties).map(([key, prop]) => prop.elements);
    //console.log(temp);
    let script = Object.entries(properties).sort((a,b) => a>b?1:-1).map(([_,prop]) => {
      const { attrName, propName, privatePropName, type, setHostAttr, elements = {}, min, max } = prop;
      let bindingList = [];
      Object.entries(elements).forEach(([elName, data]) => {
        const { binding, conditional, forLoop, elementName, elementStr } = data;
          ...
          switch(b.type) {
            case PROPERTY_TYPES.ATTR:
              if (b.inForLoop) {
                ...
                line.push(`this.allEls('${elementName}').forEach(el => setAttr( el, '${b.attribute}', ${this.#getPipedVal(b.pipes, b.default)} ));`);
              }
              else {
                line.push(`// Call 'setAttr' for the element: ${elementStr}`)
                line.push(`setAttr( ${elName}, '${b.attribute}', ${this.#getPipedVal(b.pipes, b.default)} );`);
              }
              break;
            ...
            case PROPERTY_TYPES.ARIA:
              if (b.inForLoop) {
                line.push(`// Call 'setAttr' for all of the loop generated elements: ${elementStr}`)
                line.push(`this.allEls('${elementName}').forEach(el => setAttr( el, 'aria-${b.attribute}', ${this.#getPipedVal(b.pipes, b.default)} ));`);
              }
              else {
                line.push(`// Call 'setAttr' for the element: ${elementStr}`)
                line.push(`setAttr( ${elName}, 'aria-${b.attribute}', ${this.#getPipedVal(b.pipes, b.default)} );`);
              }
              break;
            ...
            case PROPERTY_TYPES.DATA:
              if (b.inForLoop) {
                ...
                line.push(`this.allEls('${elementName}').forEach(el => el.dataset.${b.prop} = ${this.#getPipedVal(b.pipes, b.default)});`);
              }
              else {
                line.push(`// Set 'dataset.${b.prop}' for the element: ${elementStr}`)
                line.push(`${elName}.dataset.${b.prop} = ${this.#getPipedVal(b.pipes, b.default)};`);
              }
              break;
            ...
            case PROPERTY_TYPES.PROP:
              if (b.inForLoop) {
                ...
                line.push(`this.allEls('${elementName}').forEach(el => el.${b.prop} = ${this.#getPipedVal(b.pipes, b.default)});`);
              }
              else {
                line.push(`// Set the property '${b.prop}' for the element: ${elementStr}`)
                line.push(`${elName}.${b.prop} = ${this.#getPipedVal(b.pipes, b.default)};`);
              }
              break;
...
//************************************************************************************
// Transpile variables
const __buildDate = new Date('${COMPILE_DATE_ISO}') // When this component was built
const __buildVersion = '0.0.0' // The build version that generated this file
      ...
      if (typeof el === 'object') {
        ...
        bindings?.forEach(binding => {
          if (binding.add2WayBinding) {
            const fnName = `#${binding.srcField}_onInputHandler_${this.#inputHandlerIndex++}`.replace(/##/g, '#_');
        ...
        if (children.length > 0) {
          ...
          let [moreEventList, moreInputList] = this.#buildEventList(children)
          // do not use eventList.push(...moreEventList)
          // It causes a stack overflow.
          eventList = [...eventList, ...moreEventList];
          inputHandlerList = [...inputHandlerList, ...moreInputList];

          // Clean up memory to avoid stack overflow
          moreEventList = moreInputList = null;
        }
