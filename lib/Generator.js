...
` : ''; 
  ...
  #getPipedVal({pipes, setterProp, defaultVal = null}) {
    if (pipes) {
      return `${[...pipes].reverse().join('(')}(${setterProp} ?? ${defaultVal}${pipes.map(p => ')').join('')}`;
    }
...
${classScript}`;
  ...
  #processPropertyScript(properties) {
    // ******************************************************************************
    // ******************************************************************************
    // The loopItemHandlerName should be created when we are processing the $for
    // element and be passed into this function.
    // It should also be used in the funcion #processPropertyScript in Generator.js
    // ******************************************************************************
    // ******************************************************************************
    let script = Object.entries(properties).sort(this.#sortPropertyEntities).map(([_,prop]) => {
      ...
      Object.entries(elements).forEach(([elName, data]) => {
        ...
        if (forLoop) {
          //console.log('================ forLoop');
          //console.log(forLoop);
          const { forEventInfo, bindings } = forLoop;
          const usesEvents = forEventInfo.length > 0;
          const eventsArg = usesEvents ? ', events' : '';
          const elFieldName = elName.split('.').slice(-1)[0];
          //const loopItemHandlerName = `#${propName}_loopHandler_${loopHandlerIndex}`;
          const loopItemHandlerName = '#loopHandler_0';
  ...
  #makeAttrLine(line, elementStr, elName, binding, spacing='') {
    ...
    line.push(`${spacing}setAttr( ${elName}, '${binding.attribute}', ${this.#getPipedVal(binding)});`);
  }

  #makeAriaLine(line, elementStr, elName, binding, spacing='') {
    line.push(`${spacing}// Call 'setAttr' for the element: ${elementStr}`);
    line.push(`${spacing}setAttr( ${elName}, 'aria-${binding.attribute}', ${this.#getPipedVal(binding)});`);
  }

  #makeDataLine(line, elementStr, elName, binding, spacing='') {
    line.push(`${spacing}// Set 'dataset.${binding.prop}' for the element: ${elementStr}`);
    line.push(`${spacing}${elName}.dataset.${binding.prop} = ${this.#getPipedVal(binding)};`);
  }
  ...
  #makePropLine(line, elementStr, elName, binding, spacing='') {
    ...
    line.push(`${spacing}${elName}.${binding.prop} = ${this.#getPipedVal(binding)};`);
  }
      ...
      switch (b.type) {
        case PROPERTY_TYPES.ATTR:
          if (b.inForLoop) {
            ...
            line.push(`this.allEls('${elementName}').forEach(el => setAttr( el, '${b.attribute}', ${this.#getPipedVal(b)}));`);
          }
          else {
        ...
        case PROPERTY_TYPES.ARIA:
          if (b.inForLoop) {
            line.push(`// Call 'setAttr' for all of the loop generated elements: ${elementStr}`);
            line.push(`this.allEls('${elementName}').forEach(el => setAttr( el, 'aria-${b.attribute}', ${this.#getPipedVal(b)}));`);
          }
          else {
        ...
        case PROPERTY_TYPES.DATA:
          if (b.inForLoop) {
            ...
            line.push(`this.allEls('${elementName}').forEach(el => {el.dataset.${b.prop} = ${this.#getPipedVal(b)}});`);
          }
          else {
        ...
        case PROPERTY_TYPES.PROP:
          if (b.inForLoop) {
            ...
            line.push(`this.allEls('${elementName}').forEach(el => {el.${b.prop} = ${this.#getPipedVal(b)}});`);
          }
          else {
...
export const __buildDate = new Date('${COMPILE_DATE_ISO}')
// The build version that generated this file
export const __buildVersion = '0.0.0'
