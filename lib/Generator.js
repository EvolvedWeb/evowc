...
export class Generator {
  ...
  set ${propName}(newVal) {${typeConversion}
    ...
    if(${comparison}) {
      ...
      this.callUpdate({cpa: '${propName}', oldVal, newVal});
    }
  }`;
    }).join('\n');
...
` : '';
    ...
    const importList = getImports(properties, rootScript, classScript, usesAel, !!css);
...
const appendStylesFn = createStyles(styles, componentName);`;
  }
