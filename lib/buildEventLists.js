...
function generateList(events, element, bindings, eventList, inputHandlerList, forLoopItem) {
  ...
  Object.entries(events ?? {}).forEach(([evt, fn]) => {
    if (evt === 'input' && has2WayBinding) {
      ...
      // TODO: 2023-09-21 - Add code to handle P,S,I
      const fnParts = fn.split(':');
      fn = fnParts[0];
      // TODO: 2023-09-21 - Validate that this function exists
      const codeLines = [];

      if(fnParts[1]) {
        codeLines.push(...fnParts[1].split('').map(option => {
          option = option.trim();
      ...
      if (forLoopItem) {
        codeLines.push(`this.${fn}(evt, {...evt.target.dataset}, _index, ${forLoopItem})`);
      }
      else {
        codeLines.push(`this.${fn}(evt, {...evt.target.dataset})`);
      }
   ...
   * @param {object} data - The values for "evt.target.dataset"
   `;
      if( forLoopItem ) {
        eventList.push(`ael( ${element}, 'input', (evt)=>this.${fnName}(evt, {...evt.target.dataset}, _index, ${forLoopItem}))`);
...
function buildEventLists(elementList) {
  ...
  function buildForEventList(elList) {
    elList.forEach(el => {
      if (typeof el === 'object') {
  ...
  function buildRegularEventList(elList) {
    elList.forEach(el => {
      if (typeof el === 'object') {
